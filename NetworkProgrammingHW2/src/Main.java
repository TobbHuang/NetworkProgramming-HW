...
public class Main {
	...
	static String IP;
	static int PORT;
	static String DESTINATION_IP = "127.0.0.1";
	static int DESTINATION_PORT = 5566;
	static String HELPSTR = "Enter keyboard:\n"
				...
				while (true) {
					...
					if (cmd.equals("end")) {
						...
						if (cmd.equals("w")) {
							...
							jsn.put("action", "unDo");
						} else if (cmd.equals("whosyourdaddy")) {
							jsn.put("action", "whosyourdaddy");
						} else {
							// 如果命令不对
							System.out.print("move>");
							continue;
						}
						JSONObject rcvJsn = new JSONObject(tcpConnect(s, is,
								os, jsn.toString()));
						// System.out.println(rcvJsn.toString());
						// 这个地方有点坑，status突然变成boolean了，然后后来改正了
						if (rcvJsn.getInt("status") == 1) {
							// print
							String[] data = rcvJsn.getString("message").split(
									",");
